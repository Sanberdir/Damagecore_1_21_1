...
public final class ExplosionProtectionHelper {
    ...
    public static float EXPLOSION_PROTECTION_PER_LEVEL = 0.15f; // 15% за уровень
    ...
    public static float getEnchantProtectionPercentForStack(ItemStack stack, LivingEntity entity, DamageType type) {
        ...
        Holder<Enchantment> blastProtectionHolder = entity.registryAccess()
                .registryOrThrow(Registries.ENCHANTMENT)
                .getHolderOrThrow(Enchantments.BLAST_PROTECTION);

        int level = stack.getEnchantmentLevel(blastProtectionHolder);
    ...
    public static float getEnchantProtectionPercent(LivingEntity entity, DamageType type) {
        ...
        // ✅ ИСПРАВЛЕНО ДЛЯ 1.21.1: Получаем Holder чар Взрывоустойчивости из реестра игры
        Holder<Enchantment> blastProtectionHolder = entity.registryAccess()
                .registryOrThrow(Registries.ENCHANTMENT)
                .getHolderOrThrow(Enchantments.BLAST_PROTECTION);

        // ✅ ИСПРАВЛЕНО ДЛЯ 1.21.1: Передаем Holder в EnchantmentHelper
        int totalLevel = EnchantmentHelper.getEnchantmentLevel(blastProtectionHolder, entity);
